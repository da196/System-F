...
function getContent(){
	...
	document.getElementById("overlay").style.display = "block";
	 $.ajax({
	     type: "post",
	      url: "actions/components/search_content_pl.jsp", //this is my servlet
	       data: {
	    	   file_date: file_date,
...
function getProgramLineUp(){
	...
	$.ajax({ 
	     type: "GET",
	      url: "actions/components/getProgramLineUp.jsp", //this is my servlet
	      dataType: "json",
	       data: {
	    	   file_date: file_date,
	    	   channel_id: channel_id
		          ...
		          success: function(programList){  
			          var s = '';
			          s += '<div id="program" class="form-group col-12">';
			          s += '<label for="program"><strong>Program Line up </strong></label>';
			          s += '<select class="form-control" name="programID" id="programID" >';

			          $.each(programList.data, function(index,programList) {  
			        	 // alert(programList.id);
			                s += '<option value="'+programList.id+'">' + programList.programeName + '</option>	';
			          });	
				      			   ...
				      			   <div class="form-group row">
										...
										ErrorLogsAppender.appendException("Search.jsp page", er, errorLogs);
			      			            ...
			      			            <div class="form-group col-sm-3">
			      			  			...
			      			  			<input class="form-control" id="inputDate" type="date" name="inputDate" onChange="getProgramLineUp();">  
			      			            </div>	
